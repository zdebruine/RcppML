...
- **L1 (LASSO)**: Produces element-wise sparsity. Ideal when each factor should activate only a subset of features.
- **L2 (Ridge)**: Shrinks coefficients smoothly. Prevents individual loadings from dominating.
- **L21 (Group LASSO)**: Zeros entire groups — useful for feature selection across factors.
- **Angular**: Penalizes $\cos(w_i, w_j)$ between factor pairs, pushing them toward orthogonality.
- **Graph Laplacian**: Adds $\lambda \, \text{tr}(H L H^\top)$ where $L = D - A$ is the graph Laplacian. Connected nodes receive similar factor values.
...
Graph-regularized NMF produces spatially coherent community patterns — nearby grid cells load similarly on each factor, reflecting the geographic structure of bird communities. The smoothness metric decreases substantially with only a modest increase in reconstruction error.
