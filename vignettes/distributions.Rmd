...
1. **Select distribution**: Use `auto_nmf_distribution()` or `score_test_distribution()` to find the best variance function. Start with `dispersion = "none"` and `zi = "none"`.
...
> **Tip**: The `distribution` parameter (available in `auto_nmf_distribution()`) can be set to `"auto"` for automatic distribution selection. This evaluates multiple candidates and selects the best one by the specified criterion (default: BIC).
...
> **Not all sparse data has excess zeros.** For example, `hawaiibirds` is 97% sparse but `diagnose_zero_inflation()` finds an excess zero rate below 1% — the zeros are well-explained by the count distribution itself. Zero-inflation modeling adds value only when the diagnostic confirms structural excess zeros.
