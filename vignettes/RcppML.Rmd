...
The 'RcppML' package is intended for high-performance machine learning using Rcpp with a focus on matrix factorization.
...
* Control over least squares solvers, i.e. FAST and coordinate descent methods, see \code{\link{nnls}}

The following example runs rank-10 NMF for a 90% sparse random 10,000 x 10,000 matrix:

```{R}
A <- rsparsematrix(10000, 10000, 0.1)

model <- RcppML::nmf(A, 10)
w <- model$w
d <- model$d
h <- model$h
...
model <- RcppML::nmf(A_sym, 10, symmetric = TRUE)
```
...
```{R}
model <- RcppML::nmf(A_sym, 10, symmetric = TRUE, calc_mse = TRUE)
```
