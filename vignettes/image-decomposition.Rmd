...
```{r swimmer-source, fig.width = 7, fig.height = 2}
# Show a few raw swimmer images
source_data <- do.call(rbind, lapply(c(1, 50, 100, 200), function(img_idx) {
  vals <- A[, img_idx]
  grid <- expand.grid(x = 1:32, y = 1:32)
  grid$value <- vals
  grid$image <- paste("Image", img_idx)
  grid
}))

ggplot(source_data, aes(x = x, y = y, fill = value)) +
  geom_raster() +
  ...
  ggtitle("Sample swimmer images (32 × 32)")
```
