...
RcppML implements multiple SVD solvers, selectable via the `method` parameter:

| Method | Description | Constraints | Best For |
|--------|-------------|-------------|----------|
| `"deflation"` | Sequential component extraction | Sparse, nonneg, L1/L2 | Constrained PCA, nonneg PCA |
| `"krylov"` / `"lanczos"` | Tri-diagonal decomposition | Sparse, nonneg | Moderate-sized matrices |
| `"irlba"` | Implicitly restarted Lanczos | Sparse, nonneg | Large matrices (memory-efficient) |
| `"randomized"` | Stochastic projection | None | Very large matrices (fastest) |
| `"auto"` | Automatic selection | — | Default: picks based on matrix size and constraints |

**Guidance**: Use `"auto"` (default) for most cases. Use `"deflation"` when nonneg or sparse constraints are required. Use `"irlba"` for very large matrices where memory is a concern. Use `"randomized"` for quick approximations on massive data.
...
The sparse PCA loadings are concentrated at zero with a few large values, while standard PCA loadings are spread more uniformly — illustrating the sparsity-variance tradeoff.

### Example 3: Nonneg PCA vs. Standard PCA

Nonneg PCA constrains loadings to be nonneg, producing parts-like components similar to NMF while maintaining the PCA framework.
...
Each method offers a distinct tradeoff: standard PCA maximizes variance explained, sparse PCA maximizes loading interpretability, and nonneg PCA produces parts-like loadings while maintaining orthogonality. Choose based on whether you need maximum fidelity (standard), feature selection (sparse), or parts-based interpretation (nonneg).
