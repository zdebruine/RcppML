testConvergence <- function(A, k, mask_zeros = FALSE, nonneg = TRUE, ...){
  m0 <- nmf(A, k, v = F, tol = 1e-10, maxit = 1, mask_zeros = mask_zeros, ...)
  m1 <- nmf(A, k, v = F, tol = 1e-10, maxit = 10, mask_zeros = mask_zeros, ...)
  # test convergence
  expect_gt(mse(m0, A, mask_zeros), mse(m1, A, mask_zeros))
