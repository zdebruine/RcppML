test_that("Testing RcppML::solve solutions", {
  ...
  expect_equal(true_solution, RcppML::solve(a, b), tolerance = 1e-6)

  # nnls solution should be equal to true solution
  expect_equal(true_nnls_solution, RcppML::solve(a, b, nonneg = TRUE, maxit = 1000, tol = 1e-9))

  # solution from a parallelized matrix "b" should be the same as the unparallelized vector "b"
  expect_equal(RcppML::solve(a, b_matrix[,2]), RcppML::solve(a, b_matrix)[,2])
  
  # L1 regularization scaled to max(b) should give all zeros
  expect_equal(sum(RcppML::solve(a, b, L1 = max(b), nonneg = TRUE)), 0)
  
  # check that float and double precision give the same solution (within a tolerance)
  expect_equal(RcppML::solve(a, b, precision = "double"), RcppML::solve(a, b, precision = "float"), tolerance = 1e-5)

  # check that coordinate descent only gives same solution as FAST + CD
  expect_equal(RcppML::solve(a, b, x = runif(5), maxit = 1000, nonneg = TRUE), RcppML::solve(a, b, maxit = 1000, nonneg = TRUE))  

  # check that incompatible sizes give an error
  expect_error(RcppML::solve(a, as.vector(1:3)));
})