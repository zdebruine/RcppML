test_that("Testing RcppML::project", {
  ...
  # project and test that error of solved model is better than error of a random model (DENSE)
  beta_hat <- project(x, y, threads = 1)
  ...
  # test that nonneg successfully enforces non-negativity constraints (DENSE)
  beta_hat <- project(x, y, threads = 1)
  expect_equal(sum(beta_hat[beta_hat < 0]), 0)
  
  # test that error gets better after several alternating non-negative iterations (DENSE)
  A <- rsparsematrix(100, 100, 0.5)
  H_initial <- matrix(runif(100 * 5), 5, 100)
  W_initial <- t(project(t(A), t(H_initial), threads = 1))
  H <- project(A, W_initial, threads = 1)
  for(i in 1:10){
    W <- t(project(t(A), t(H), threads = 1))
    H <- project(A, W, threads = 1)
  }
  ...
  # test that error gets better after several alternating non-negative iterations (SPARSE)
  A <- rsparsematrix(100, 100, 0.5)
  H_initial <- matrix(runif(100 * 5), 5, 100)
  W_initial <- t(project(t(A), t(H_initial), threads = 1))
  H <- project(A, W_initial, threads = 1)
