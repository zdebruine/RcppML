...
#' @param seed Random seed for initialization and cross-validation. Default: 0 (random).
...
#' @param test_fraction Fraction of entries to hold out for auto-rank. Default: 0.05.
#' @param cv_seed Separate seed for holdout mask. Default: 0 (derive from \code{seed}).
...
#' @param method Algorithm to use. One of \code{"deflation"} (default), \code{"krylov"},
...
svd <- function(A, k = 10, tol = 1e-5, maxit = 200,
                ...
                seed = 0, threads = getOption("RcppML.threads", 0L),
                L1 = 0, L2 = 0, nonneg = FALSE, upper_bound = 0,
                L21 = 0, angular = 0,
                graph_U = NULL, graph_V = NULL, graph_lambda = 0,
                convergence = "factor",
                test_fraction = 0.05, cv_seed = 0, patience = 3,
                mask_zeros = FALSE, k_max = 50,
                resource = "auto", method = "deflation") {
  ...
  if (method == "lanczos" && missing(maxit)) maxit <- 0L
  if (method == "randomized" && missing(maxit)) maxit <- 3L
  if (method == "irlba" && missing(maxit)) maxit <- 0L
  if (method == "irlba" && missing(tol)) tol <- 1e-5
  if (method == "krylov" && missing(maxit)) maxit <- 0L

  # =========================================================================
  # Validate method
  # =========================================================================

  valid_methods <- c("deflation", "krylov", "lanczos", "irlba", "randomized")
