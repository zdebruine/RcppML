# nmf_gpu_baseline.R — GPU NMF benchmark suite
#
# Benchmarks MSE NMF on GPU with sparse and dense inputs.
# k in {8, 16, 32, 64}, 20 iterations, 5 replicates.

run_suite <- function(datasets, metadata) {
  results <- list()

  if (!isTRUE(tryCatch(RcppML:::gpu_available(), error = function(e) FALSE))) {
    cat("  GPU not available, skipping suite\n")
    return(results)
  }
