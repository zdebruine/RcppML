...
Thank you for your interest in contributing to RcppML! This document
...
1.  Edit files in `R/` with roxygen2 comments (`#' @param`,
    `#' @export`, etc.)
...
(subdivided into `RcppML/`, `FactorNet/`, `streampress/`, and
`sparsepress/`). The `src/` directory contains Rcpp bridge files
(`RcppFunctions*.cpp`) that provide `// [[Rcpp::export]]` wrappers, plus
`sparsepress_bridge.cpp` for StreamPress I/O.
...
| File(s)               | Generator                                                                                                                                                           |
|-----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `NAMESPACE`           | `roxygen2` via [`devtools::document()`](https://devtools.r-lib.org/reference/document.html)                                                                         |
| `man/*.Rd`            | `roxygen2` via [`devtools::document()`](https://devtools.r-lib.org/reference/document.html)                                                                         |
| `src/RcppExports.cpp` | [`Rcpp::compileAttributes()`](https://rdrr.io/pkg/Rcpp/man/compileAttributes.html) via [`devtools::document()`](https://devtools.r-lib.org/reference/document.html) |
| `R/RcppExports.R`     | [`Rcpp::compileAttributes()`](https://rdrr.io/pkg/Rcpp/man/compileAttributes.html) via [`devtools::document()`](https://devtools.r-lib.org/reference/document.html) |

## Code Style

### R Code

- Follow standard R style (snake_case for functions, PascalCase for S4
  classes)
- All exported functions must have complete roxygen documentation
  including `@return` and `@examples`
- Use [`stop()`](https://rdrr.io/r/base/stop.html) for user-facing
...
- Headers must be Rcpp-free — use `RCPPML_LOG_INFO()` instead of
  `Rcpp::Rcout`
...
Open an issue at <https://github.com/zdebruine/RcppML/issues> with: - A
minimal reproducible example - R session info
