...
namespace RcppML {
  ...
  public:
    ...
    unsigned int maxit = 100, cd_maxit = 100, fast_maxit = 10, threads = 0;
    ...
    // update "h"
    void projectH(RcppML::SparseMatrix& A) { project(A, w, h, nonneg, L1_h, cd_maxit, fast_maxit, cd_tol, threads); }
    void projectH(Rcpp::NumericMatrix& A) { project(A, w, h, nonneg, L1_h, cd_maxit, fast_maxit, cd_tol, threads); }

    // update "w"
    void projectW(RcppML::SparseMatrix& A) {
      if (is_appx_symmetric(A)) project(A, h, w, nonneg, L1_w, cd_maxit, fast_maxit, cd_tol, threads);
      else projectInPlace(A, h, w, nonneg, L1_w, cd_maxit, fast_maxit, cd_tol, threads);
    }
    ...
    void fit(RcppML::SparseMatrix& A) {
      ...
      for (; iter_ < maxit; ++iter_) { // alternating least squares updates
        ...
        // update "h"
        project(A, w, h, nonneg, L1_h, cd_maxit, fast_maxit, cd_tol, threads);
        if (diag) scaleH(); // reset diagonal and scale "h"

        // update "w"
        if (is_A_symmetric) project(A, h, w, nonneg, L1_w, cd_maxit, fast_maxit, cd_tol, threads);
        else if (updateInPlace) projectInPlace(A, h, w, nonneg, L1_w, cd_maxit, fast_maxit, cd_tol, threads);
        else project(At, h, w, nonneg, L1_w, cd_maxit, fast_maxit, cd_tol, threads);
