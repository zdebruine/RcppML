    ...
    if (method == "euclidean") {
        ...
        for (unsigned int i = 0; i < B.cols(); ++i) {
            for (unsigned int j = 0; j < A.cols(); ++j) {
                ...
                for (RcppSparse::Matrix::InnerIterator it(A, j); it; ++it)
    ...
    if (method == "euclidean") {
        ...
        for (unsigned int i = 0; i < B.cols(); ++i) {
            for (unsigned int j = 0; j < A.cols(); ++j) {
                ...
                for (RcppSparse::Matrix::InnerIterator it(B, j); it; ++it)
                    dist += it.value() * it.value();
                RcppSparse::Matrix::InnerIterator it_A(A, j), it_B(B, i);
                while (it_A && it_B) {
                    if (it_A.row() < it_B.row()) ++it_A;
                    else if (it_A.row() > it_B.row()) ++it_B;
                    else {
...
inline Eigen::MatrixXd distance(RcppSparse::Matrix& A, std::string method, const unsigned int threads) {
    ...
    if (method == "euclidean") {
        ...
        for (unsigned int i = 0; i < (A.cols() - 1); ++i) {
            for (unsigned int j = (i + 1); j < A.cols(); ++j) {
                ...
                RcppSparse::Matrix::InnerIterator it1(A, i), it2(A, j);
                while (it1 && it2) {
