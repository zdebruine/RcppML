  ...
  void svd<Eigen::MatrixXd>::fit_rank_k()
  {
    ...
    for (; iter_ < maxit; ++iter_)
    {
      ...
      for (int _k = 0; _k < k; ++_k)
        a(_k) = u.col(k).dot(u.col(_k)) / (akk + DIV_OFFSET);
...
#pragma omp parallel for num_threads(threads) schedule(dynamic)
      ...
      for (int i = 0; i < v.rows(); ++i)
      {
        ...
        // v(i, k) /=  (u.col(k).dot(u.col(k)) + DIV_OFFSET);
      }
      ...
      for (int _k = 0; _k < k; ++_k)
        a(_k) = v.col(k).dot(v.col(_k)) / (akk + DIV_OFFSET);
...
#pragma omp parallel for num_threads(threads) schedule(dynamic)
      ...
      for (int i = 0; i < u.rows(); ++i)
      {
        ...
        // u(i, k) /= (v.col(k).dot(v.col(k)) + DIV_OFFSET);
      }
